...
export const BottomSheetPortal = ({
  ...
  const points = memoizedSnapPoints
    ? (Array.isArray(memoizedSnapPoints)
      ? memoizedSnapPoints
      : 'value' in memoizedSnapPoints
      ? (memoizedSnapPoints.value as Array<string | number>)
      : [])
    : [];

  const validIndex =
    points && points.length > 0
      ? Math.min(currentIndex, points.length - 1)
      : currentIndex;
  ...
  return (
    <Overlay isOpen={true} isKeyboardDismissable={false} style={{ flex: 1 }}>
      <StyledGorhomBottomSheet
        ...
        // @ts-ignore
        handleIndicatorClassName={`${handleIndicatorClassName || 'bg-neutral-500'}`}
        {...props}
      >
        {props.children}
      </StyledGorhomBottomSheet>
...
export const BottomSheetItem = ({
  ...
  const Pressable = (Platform.OS === 'web' ? RNPressable : StyledGGHPressable) as any;
