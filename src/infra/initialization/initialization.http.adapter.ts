  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (bimestersRes.status === "fulfilled" && bimestersRes.value?.data) {
        ...
        console.warn(
          "Failed to load bimesters during initialization:",
          bimestersRes,
        );
      }
      ...
      if (schoolsRes.status === "fulfilled" && schoolsRes.value?.data) {
        ...
        console.warn(
          "Failed to load schools during initialization:",
          schoolsRes,
        );
      }
      ...
      if (selectedSchoolId) {
        const classesRes = await this.http.get<Class[]>(
          `/classes?schoolId=${selectedSchoolId}`,
        );
        if (classesRes.data) {
